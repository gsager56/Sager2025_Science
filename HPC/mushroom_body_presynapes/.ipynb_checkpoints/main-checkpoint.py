...
def main(_):
    ...
    coords_roi = fetch_synapses(NC(bodyId=pre_bodyId), SC(primary_only=False, type='pre'))[['x','y','z','roi']]
    coords_roi = coords_roi.iloc[ np.where( np.isin(coords_roi['roi'].to_numpy(), compartments) )[0] ]

    synapse_df = utils.get_synapse_df( pre_bodyId, 'pre')
    ...
    pre_on = np.zeros( synapse_df.shape[0] )
    for i_synapse in range(len(synapse_df)):
        _, mito_CS = voxel_utils.get_CrossSection_info(pre_bodyId, syn_coords[i_synapse], 50, return_mito = True)
    ...
    for i_synapse in range(len(synapse_df)):
        for i_post in range(len(synapse_df.iloc[i_synapse]['connecting_type'])):
            ...
            feat_vec.append( np.append(synapse_df.iloc[i_synapse][ ['type', 'confidence', 'x', 'y', 'z', 'roi_compartment'] ].to_numpy(), 
                                        np.array([post_type, post_bodyId, pre_on[i_synapse]], dtype=object)) )
    final_synapse_df = pd.DataFrame(np.array(feat_vec), columns = cols)
    final_synapse_df.to_csv(home_dir + f'/saved_data/MB_presynapses/{pre_bodyId}_{pre_type}.csv', index=False)
