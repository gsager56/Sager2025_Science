...
log_file = 'joblist_KC_MBON.txt'
with open(log_file, 'w') as f:
    f.truncate()
    for row in rows:
        pre_bodyId, pre_type = all_bodyIds[row,[0,1]]
        if not isfile(home_dir + f'/saved_data/KC_MBON_epi/{pre_bodyId}_{pre_type}.csv'):
            f.write(f'module load miniconda; source activate flyem-stuff; python3 KC_MBON_main.py --row={row}\n')
