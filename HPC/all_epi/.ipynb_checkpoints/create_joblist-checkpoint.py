...
for neuron_type in np.unique( all_bodyIds[:,1] ):
    bool_ = np.all([all_bodyIds[:,1] == neuron_type, all_bodyIds[:,2].astype(int) == 0], axis=0)
    if np.sum( bool_ ) >= 5:
...
with open(log_file, 'w') as f:
    ...
    for row in rows:
        ...
        if not isfile(home_dir + f'/saved_data/all_mitoconnectome/{pre_bodyId}_{pre_type}.csv'):
