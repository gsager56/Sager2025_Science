...
def main(_):
    ...
    coords_roi = fetch_synapses(NC(bodyId=bodyId), SC(primary_only=False, type='post'))[['x','y','z','roi']]
    coords_roi = coords_roi.iloc[ np.where( np.isin(coords_roi['roi'].to_numpy(), compartments) )[0] ]

    synapse_df = utils.get_synapse_df( bodyId, 'post')
    ...
    final_synapse_df.to_csv(home_dir + f'/saved_data/MB_postsynapses/{bodyId}_{neuron_type}.csv', index=False)
