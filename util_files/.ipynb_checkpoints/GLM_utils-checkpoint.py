...
def get_mito_position_space(neuron_types, arbors, offset = 0, jitter_strength = 0):
    '''
    Inputs:
        - neuron_types : array of neuron_types to compute the feature space
        - arbors : list of arbor to compute feature space
        - offset : value to assign to features that cannot be determined for a certain point
        - jitter_strength : distance to jitter the mitochondria
    Outputs:
        - all_X : n x m feature space array
        - all_Y : n x 1 boolean array of labels for all_X
        - bodyId_type_arbor : n x 3 array of which bodyId, neuron type, and arbor each point came from
    '''
